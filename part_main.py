...
model_str='cifar10_part_coteaching_'+args.noise_type+'_'+str(args.noise_rate)+("-%s.txt" % args.seed)
...
def main():
    ...
    max_pt=np.zeros(2)
    hyphyp=np.ones(4)
    hypgrad=np.zeros((4,1))
    fisher=np.zeros((4,4))
    for iii in range(args.n_iter):
        print('Distribution:',hyphyp)
        cur_param=np.zeros((args.n_samples,2))
        loggrad=np.zeros((args.n_samples,4,1))
        ...
        cur_param=np.zeros(2)
        loggrad=np.zeros((4,1))
